...
vec3 reflectance = 1.1f * vec3(1, 1, 1);
// ----------------------------------------------------------------------------
// FUNCTIONS
...
void Draw()
{
	...
	for (int i = 0; i < triangles.size(); ++i)
	{
		...
		vertices[0].reflectance = reflectance;
		vertices[1].reflectance = reflectance;
		vertices[2].reflectance = reflectance;
...
void VertexShader(const Vertex& v, Pixel& p) {
	...
	p.zinv = 1.0f / P.z;
	p.x = (f * P.x * p.zinv) + (SCREEN_WIDTH / 2);
	p.y = (f * P.y * p.zinv) + (SCREEN_HEIGHT / 2);

	float r = glm::length(lightPos - v.position);
	vec3 r_hat = glm::normalize(lightPos - v.position);
	vec3 n_hat = glm::normalize(v.normal);
	vec3 D = vec3(lightPower * glm::max(r_hat * n_hat, 0.f)) / (4.f * 3.14f * glm::dot(r, r));
